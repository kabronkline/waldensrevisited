...
export async function handleApi(request, env, session) {
  ...
  // GET /api/surveys — list active surveys with user's responses
  if (path === '/api/surveys' && method === 'GET') {
    const { results: surveys } = await env.DB.prepare(
      `SELECT s.*, u.name as author_name, u.role as author_role
       FROM surveys s JOIN users u ON s.user_id = u.id
       WHERE s.is_active = 1
       ORDER BY s.created_at DESC`
    ).all();

    // Fetch user's responses for each survey
    for (const survey of surveys) {
      const { results: responses } = await env.DB.prepare(
        'SELECT selected_option FROM survey_responses WHERE survey_id = ? AND user_id = ?'
      ).bind(survey.id, userId).all();
      survey.my_votes = responses.map(r => r.selected_option);

      // Fetch vote counts per option
      const { results: counts } = await env.DB.prepare(
        'SELECT selected_option, COUNT(*) as count FROM survey_responses WHERE survey_id = ? GROUP BY selected_option'
      ).bind(survey.id).all();
      survey.vote_counts = counts;

      const totalRes = await env.DB.prepare(
        'SELECT COUNT(DISTINCT user_id) as total FROM survey_responses WHERE survey_id = ?'
      ).bind(survey.id).first();
      survey.total_voters = totalRes.total;
    }
    return json(surveys);
  }

  // POST /api/surveys — create survey (officers/contributors/admins only)
  if (path === '/api/surveys' && method === 'POST') {
    if (!canAutoApprovePost(session.user.role)) {
      return json({ error: 'Only officers, contributors, and admins can create surveys' }, 403);
    }
    const body = await request.json();
    if (!body.title?.trim()) return json({ error: 'Survey title is required' }, 400);
    if (!Array.isArray(body.options) || body.options.length < 2) {
      return json({ error: 'At least 2 options required' }, 400);
    }
    if (body.options.length > 10) return json({ error: 'Maximum 10 options' }, 400);

    const result = await env.DB.prepare(
      'INSERT INTO surveys (user_id, title, description, options, allow_multiple, closes_at) VALUES (?, ?, ?, ?, ?, ?)'
    ).bind(
      userId, body.title.trim(), body.description?.trim() || null,
      JSON.stringify(body.options.map(o => String(o).trim())),
      body.allow_multiple ? 1 : 0, body.closes_at || null
    ).run();
    return json({ id: result.meta.last_row_id, success: true }, 201);
  }

  // POST /api/surveys/:id/vote — cast a vote
  const surveyVoteMatch = path.match(/^\/api\/surveys\/(\d+)\/vote$/);
  if (surveyVoteMatch && method === 'POST') {
    const surveyId = parseInt(surveyVoteMatch[1]);
    const survey = await env.DB.prepare('SELECT * FROM surveys WHERE id = ? AND is_active = 1').bind(surveyId).first();
    if (!survey) return json({ error: 'Survey not found or closed' }, 404);

    if (survey.closes_at && new Date(survey.closes_at) < new Date()) {
      return json({ error: 'Survey has closed' }, 400);
    }

    const body = await request.json();
    const options = JSON.parse(survey.options);
    const selections = Array.isArray(body.selected) ? body.selected : [body.selected];

    if (!survey.allow_multiple && selections.length > 1) {
      return json({ error: 'Only one selection allowed' }, 400);
    }

    for (const sel of selections) {
      if (typeof sel !== 'number' || sel < 0 || sel >= options.length) {
        return json({ error: 'Invalid option' }, 400);
      }
    }

    // Remove previous votes if single-select
    if (!survey.allow_multiple) {
      await env.DB.prepare('DELETE FROM survey_responses WHERE survey_id = ? AND user_id = ?').bind(surveyId, userId).run();
    }

    for (const sel of selections) {
      await env.DB.prepare(
        'INSERT OR IGNORE INTO survey_responses (survey_id, user_id, selected_option) VALUES (?, ?, ?)'
      ).bind(surveyId, userId, sel).run();
    }
    return json({ success: true });
  }

  // PUT /api/surveys/:id/close — close a survey (creator or officer/admin)
  const surveyCloseMatch = path.match(/^\/api\/surveys\/(\d+)\/close$/);
  if (surveyCloseMatch && method === 'PUT') {
    const surveyId = parseInt(surveyCloseMatch[1]);
    ...
    if (survey.user_id !== userId && !isOfficerOrAdmin(session.user.role)) {
      return json({ error: 'Not authorized' }, 403);
    }

    await env.DB.prepare("UPDATE surveys SET is_active = 0, updated_at = datetime('now') WHERE id = ?").bind(surveyId).run();
    return json({ success: true });
  }

  // DELETE /api/surveys/:id — delete survey (creator or admin)
  const surveyDeleteMatch = path.match(/^\/api\/surveys\/(\d+)$/);
  if (surveyDeleteMatch && method === 'DELETE') {
    const surveyId = parseInt(surveyDeleteMatch[1]);
    const survey = await env.DB.prepare('SELECT * FROM surveys WHERE id = ?').bind(surveyId).first();
    if (!survey) return json({ error: 'Survey not found' }, 404);

    if (survey.user_id !== userId && session.user.role !== 'admin') {
      return json({ error: 'Not authorized' }, 403);
    }
